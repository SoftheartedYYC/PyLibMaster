...
// ============ Mock 基础设施 ============
// 通过 require.cache 注入模块 mock，拦截 pipManager 的依赖
...
describe('installPackages', () => {
  ...
  // 场景 1: 正常安装成功
  it('installs packages successfully via runPip', async () => {
    // runPip 默认返回成功（mock 基础设施）
    ...
    // 验证 runPip 被调用（每个包至少调用 1 次，默认镜像尝试 2 次）
    const installCalls = mockState.processRunner.runPipCalls.filter(
  ...
  it('retries on alternate mirrors when default mirror fails', async () => {
    let callCount = 0;
    mockState.processRunner.runPipHandler = async (pythonPath, args, options) => {
      callCount++;
