...
## 更新摘要
**变更内容**   
- 增强了 backupManager.js 的完整备份/恢复功能，使用 pip freeze 格式、安全备份 ID 验证、基于时间戳的命名和强制重装能力
- 新增 templateManager.js 用于项目模板和环境快照管理
- 新增 auditManager.js 用于安全漏洞扫描和审计
- 新增 venvManager.js 用于虚拟环境管理
- 更新了 IPC 处理器以支持所有新功能模块
...
- [core/operations/venvManager.js:1-80](file://core/operations/venvManager.js#L1-L80)
...
- 虚拟环境管理（venvManager）：创建/删除虚拟环境、获取环境信息、支持不同 Python 版本和 pip 配置。
...
### 虚拟环境管理（venvManager）
- 使用场景
  - 创建隔离的 Python 开发环境，避免包冲突。
  - 管理多个 Python 版本和项目环境。
  - 快速获取环境信息和包列表。
- 操作流程
  - 创建环境：createVenv(options, onOutput) 支持指定 Python 路径、pip 配置、系统包继承。
  - 列出环境：listVenvs() 返回所有虚拟环境的详细信息。
  - 删除环境：deleteVenv(name, onOutput) 安全删除并清理残留。
  - 获取信息：getVenvInfo(name) 返回 Python 版本、pip 版本、包数量等。
- 关键参数与返回值
  - createVenv({ name, pythonPath, withPip, systemSitePackages }, onOutput): 返回 { name, path, pythonPath, version }。
  - listVenvs(): 返回数组 [{ name, path, pythonPath, version, pipVersion, packageCount }]。
  - deleteVenv(name, onOutput): 返回 { success, name }。
  - getVenvInfo(name): 返回 { name, path, pythonPath, version, pipVersion, basePython }。
- 安全特性
  - 名称合法性验证（字母、数字、短横线、下划线、点）。
  - 路径遍历防护，确保操作在 venvs 目录内。
  - 创建失败时自动清理残留目录。

```mermaid
classDiagram
class VenvManager {
+createVenv(options, onOutput) Object
+listVenvs() Array
+deleteVenv(name, onOutput) Object
+getVenvInfo(name) Object
+getVenvsDir() string
...
- 虚拟环境操作失败
  - 检查 venv 名称是否合法（只允许字母、数字、短横线、下划线、点）。
  - 确认基础 Python 路径正确且可执行。
  - 查看创建失败时的清理日志。
- 镜像源不可用
...
- [core/operations/venvManager.js:200-278](file://core/operations/venvManager.js#L200-L278)
...
## 结论
PyLibMaster 通过清晰的模块化设计与稳健的底层支撑（进程管理、配置持久化、日志记录），实现了备份恢复、模板管理、安全审计、虚拟环境管理、镜像源管理与调度器管理等核心业务功能。各模块职责明确、耦合度低，配合 IPC 桥接与事件驱动，提供了从基础使用到高级配置的完整能力体系。遵循本文的最佳实践与故障排除建议，可有效提升开发效率与系统稳定性。
...
- 虚拟环境（venvManager）
  - 存储位置：storage/venvs。
  - 名称规则：字母、数字、短横线、下划线、点，最大长度 64。
  - 支持选项：withPip、systemSitePackages。
- 审计（auditManager）
...
- [core/operations/templateManager.js:23-66](file://core/operations/templateManager.js#L23-66)
- [core/operations/venvManager.js:22-24](file://core/operations/venvManager.js#L22-L24)
